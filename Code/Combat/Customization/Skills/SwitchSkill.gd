...
func show_desc_tip(owner):
	Tip.set_disp(["Instantly switches to another ally, but debuffs the user's SPD by [color=#0FF]" + str(ceil(owner.get_base_stat_val("SPD")*0.4)) + "[/color] for 3 turns"])
...
func use(user):
	...
	if not target is int:
		...
		nod.stacks.append({"stat":"SPD", "val":ceil(user.get_base_stat_val("SPD")*(-0.4)), "duration": 4})
