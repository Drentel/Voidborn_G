...
func get_desc():
	return """CON
Crit damage (CTD) increased by """ + str(stacks) + ". This bonus is increased by 1~" + str(max(lvl,2)) + " when character attacks but doesn't crit, but reset when a crit happens"
